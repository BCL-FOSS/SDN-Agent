...
class UniFiNetAPI():
    ...
    async def get_unifi_session(self):
        session = aiohttp.ClientSession()
        login_data = {
            "username": UNIFI_USERNAME,
            "password": UNIFI_PASSWORD
        }
        async with session.post(f"{UNIFI_BASE_URL}/api/login", json=login_data, ssl=False) as resp:
            await resp.text()
        return session

    async def get_unifi_clients_and_aps(self):
        session = await self.get_unifi_session()
        async with session.get(f"{UNIFI_BASE_URL}/api/s/{UNIFI_SITE}/stat/sta", ssl=False) as resp:
            clients = await resp.json()
        async with session.get(f"{UNIFI_BASE_URL}/api/s/{UNIFI_SITE}/stat/device", ssl=False) as resp:
            devices = await resp.json()
        await session.close()
        return clients["data"], devices["data"]

    def store_device_ap_history(self, mac, ap_mac, ap_name):
        timestamp = int(time.time())
        key = f"device:{mac}:history"
        entry = json.dumps({"timestamp": timestamp, "ap_mac": ap_mac, "ap_name": ap_name})
        r.rpush(key, entry)
        r.expire(key, 7 * 86400)

    def get_device_ap_history(mac):
        key = f"device:{mac}:history"
        entries = r.lrange(key, 0, -1)
        return [json.loads(entry) for entry in entries]

    async def unifi_track(self):
        clients, devices = await self.get_unifi_clients_and_aps()
        ap_map = {dev["mac"]: dev["name"] for dev in devices if dev.get("type") == "uap"}
    ...
    async def device_history(self, mac):
        history = self.get_device_ap_history(mac)
        return jsonify(history)

    async def device_graph(self, mac):
        history = self.get_device_ap_history(mac)
        nodes = {}
        edges = []
        ...
        for record in history:
            ap = record["ap_name"]
            ts = record["timestamp"]
            nodes[ap] = {"id": ap, "label": ap}
            if prev and prev != ap:
                edges.append({"from": prev, "to": ap, "label": time.strftime("%H:%M", time.localtime(ts))})
            prev = ap
